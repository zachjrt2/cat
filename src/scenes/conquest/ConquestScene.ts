...
const NUM_LANES = 10;
const MAX_VISIBLE_RANKS = 3; // Windowing: only render front combatant + 2 closest reserves per lane (max ~60 sprites)
...
export class ConquestScene {
  ...
  private startBattle(): void {
    ...
    // Spawn visible window of cats (Ranks 0..2) across 10 independent vertical lane queues
    this.spawnArmyWithReserves(state.playerLanes, 'player');
    this.spawnArmyWithReserves(state.enemyLanes, 'enemy');

    // Start single centralized animation loop for all visible sprites
  ...
  // ── Centralized Single-Loop Pixel Art Rendering Engine ───────────────────
  ...
  private renderFrame(now: number): void {
    ...
    for (const info of this.spriteMap.values()) {
      const { ctx, canvas, isMoving, side, mutation, stage, baseImg, markingImg } = info;
      if (!ctx || !baseImg || !baseImg.complete || baseImg.naturalWidth === 0) continue;

      const frameDuration = isMoving ? 50 : 130;
      if (now - info.lastFrameTime >= frameDuration) {
        info.frameIndex = (info.frameIndex + 1) % 4;
      ...
      let srcY = 0;
      if (isMoving) {
        srcX = (12 + info.frameIndex) * FRAME_SIZE;
        srcY = walkRow * FRAME_SIZE;
      } else {
        srcX = (info.frameIndex % 2) * FRAME_SIZE;
  ...
  // ── Pixel-Art Cat Canvas Sprite System (10 Lanes + Windowed Ranks) ────────
  ...
  private getRankAndLaneY(rank: number, side: 'player' | 'enemy'): { entryY: number; fightY: number } {
    if (side === 'player') {
      // Front line at ~55%, Rank 1 at ~66%, Rank 2 at ~77%
      const fightY = 55 + rank * 11;
      const entryY = 115 + rank * 12;
      return { entryY, fightY };
    } else {
      // Front line at ~35%, Rank 1 at ~24%, Rank 2 at ~13%
  ...
  private spawnSingleUnit(cat: ConquestCat | EnemyCat, rank: number, laneIdx: number, side: 'player' | 'enemy'): SpriteInfo | null {
    const isFriendly = side === 'player';
    const id = isFriendly ? (cat as ConquestCat).sourceId : (cat as EnemyCat).id;
    if (this.spriteMap.has(id)) return this.spriteMap.get(id)!;

    const arena = this.overlay.querySelector('#cq-battle-arena') as HTMLElement;
    if (!arena) return null;

    const xPct = this.getLaneX(laneIdx, rank, NUM_LANES);
    const { fightY } = this.getRankAndLaneY(rank, side);
    ...
    el.style.cssText = `left:${xPct}%;top:${fightY}%;transform:translateX(-50%);z-index:${zIdx};`;
    ...
    const spriteObj: SpriteInfo = {
      ...
      lastFrameTime: performance.now() - Math.random() * 100,
      baseImg: null,
      markingImg: null,
      mutation: (cat as ConquestCat).mutation,
      stage: (cat as ConquestCat).stage,
    };

    ConquestScene.loadImage(skinSrc).then((img) => { spriteObj.baseImg = img; });
    if (markingSrc) {
      ConquestScene.loadImage(markingSrc).then((img) => { spriteObj.markingImg = img; });
    }
  ...
  private spawnArmyWithReserves(lanes: (ConquestCat | EnemyCat)[][], side: 'player' | 'enemy'): void {
    lanes.forEach((laneQueue, laneIdx) => {
      // Windowing: only render front MAX_VISIBLE_RANKS per lane
      const visibleSlice = laneQueue.slice(0, MAX_VISIBLE_RANKS);
      visibleSlice.forEach((cat, rank) => {
        const spriteObj = this.spawnSingleUnit(cat, rank, laneIdx, side);
        if (spriteObj) {
          const { entryY, fightY } = this.getRankAndLaneY(rank, side);
          spriteObj.currentY = entryY;
          spriteObj.targetY = fightY;
          spriteObj.isMoving = true;
          spriteObj.el.style.top = `${entryY}%`;
        }
      });
    });
  }
  ...
  private updateReserveAdvancement(): void {
    ...
    // 1. Advance Player Lane Queues (pure vertical queue per lane, windowed to MAX_VISIBLE_RANKS)
    playerLanes.forEach((laneQueue, laneIdx) => {
      const visibleSlice = laneQueue.slice(0, MAX_VISIBLE_RANKS);
      visibleSlice.forEach((cat, rank) => {
        if (cat.hp <= 0) return;
        let info = this.spriteMap.get(cat.sourceId);
        if (!info) {
          info = this.spawnSingleUnit(cat, rank, laneIdx, 'player') || undefined;
        }
        if (info) {
    ...
    // 2. Advance Enemy Lane Queues (pure vertical queue per lane, windowed to MAX_VISIBLE_RANKS)
    enemyLanes.forEach((laneQueue, laneIdx) => {
      const visibleSlice = laneQueue.slice(0, MAX_VISIBLE_RANKS);
      visibleSlice.forEach((enemy, rank) => {
        if (enemy.hp <= 0) return;
        let info = this.spriteMap.get(enemy.id);
        if (!info) {
          info = this.spawnSingleUnit(enemy, rank, laneIdx, 'enemy') || undefined;
        }
        if (info) {
  ...
  private animateKo(id: string): void {
    ...
    // Completely remove the defeated cat from DOM and spriteMap so no ghosts remain
    setTimeout(() => {
